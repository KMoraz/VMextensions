...
$stalecandidates = (Get-ADComputer -Filter {OperatingSystem -Like "Windows Server*"} -Property * |`
 where-object {$_.Passwordlastset -le $expiry -AND ([DateTime]::FromFileTime($_.LastLogontimestamp)) -le $expiry}|`
...
# Set outfile
$outfile = "c:\Scripts\temp\Staleservers_$((Get-Date).ToString('dd-MM-yyyy')).csv" `
...
foreach ($stalecandidate in $stalecandidates)
{
    $status = @{ "ServerName" = $stalecandidate.name;
     ...
     "Last Set"= $stalecandidate.Passwordlastset;
     "Current" = (Get-Date -f s)
